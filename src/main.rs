...
impl ResignationApp {
    ...
    fn execute_cleanup(&mut self, ctx: &Context) {
        ...
        // clean_all 的回调用于确认是否继续清理（返回 true 继续，false 取消）
...
impl App for ResignationApp {
    fn update(&mut self, ctx: &Context, _frame: &mut Frame) {
        egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.heading(RichText::new("Resignation Delete - 离职数据清理工具").size(18.0));
                ui.separator();
...
impl ResignationApp {
    ...
    fn draw_confirm_dialog(&mut self, ctx: &Context) {
        Window::new("确认清理")
            ...
            .default_width(500.0)
            .show(ctx, |ui| {
                ui.label(RichText::new("确认要清理以下数据吗？").size(16.0).strong());
                ui.add_space(8.0);
            ...
            if logs.is_empty() {
                ...
                for log in logs {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.label(RichText::new(format!("[{}]", level_str)).color(color).size(12.0));
